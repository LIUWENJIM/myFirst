...
export default function ResumeDetailPage({ resumeId, onBack, onStartInterview }: ResumeDetailPageProps) {
  ...
  if (!resume) {
    return (
      <div className="text-center py-20">
        <p className="mb-4" style={{color: 'var(--color-error)'}}>加载失败，请返回重试</p>
        <button onClick={onBack} className="px-6 py-2 rounded-lg text-white" style={{backgroundColor: 'var(--color-primary)'}}>返回列表</button>
      </div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full">
      ...
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        <div className="flex items-center gap-4">
          <motion.button
            ...
            style={{backgroundColor: 'var(--color-surface-card)', color: 'var(--color-muted)'}}
        ...
        <div className="flex gap-3">
          {detailView === 'interviewDetail' && selectedInterview && (
            <motion.button
              ...
              className="px-5 py-2.5 border rounded-lg font-medium transition-all disabled:opacity-50 flex items-center gap-2"
              style={{borderColor: 'var(--color-hairline)', backgroundColor: 'var(--color-surface-card)', color: 'var(--color-body-text)'}}
      ...
      {/* 标签页切换 */}
      {detailView !== 'interviewDetail' && (
        <div className="rounded-lg p-2 mb-6 inline-flex gap-1" style={{backgroundColor: 'var(--color-surface-card)'}}>
          {tabs.map((tab) => (
            <motion.button
              key={tab.id}
              onClick={() => handleTabChange(tab.id)}
              className="relative px-6 py-3 rounded-lg font-medium flex items-center gap-2 transition-colors"
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-2">
                ...
                {tab.count !== undefined && tab.count > 0 && (
                  <span className="px-2 py-0.5 text-xs rounded-full" style={{backgroundColor: 'var(--color-surface-cream-strong)', color: 'var(--color-primary)'}}>{tab.count}</span>
                )}
              </span>
            </motion.button>
          ))}
