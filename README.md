...
> 注：本项目用于个人学习，基于 guide 老师的开源项目，无商业用途。用 Claude Code 接入 mimo-v2.5-pro，用 design.md 提示词重构了项目的前端页面。

**InterviewGuide** 是一个基于大语言模型的智能面试辅助平台，集成了以下核心能力：

- **简历智能分析**：多格式简历解析 + AI 生成评估报告
- **模拟面试**：文字面试 + 实时语音面试，支持 Skill 驱动出题和智能追问
- **知识库 RAG**：文档上传、向量化、检索增强问答
- **面试安排**：智能解析面试邀请 + 日历管理
- **直达招聘**：大厂投递信息管理，支持分类、搜索、分页、Excel 导入
- **多模型配置**：支持 DashScope、DeepSeek、GLM 等多种 LLM Provider
...
1. **PostgreSQL + pgvector**：精简架构，PG 的向量存储功能满足 RAG 需求，无需额外引入向量数据库。
2. **Redis**：
   - 替代 `ConcurrentHashMap` 实现面试会话缓存
   - 基于 Redis Stream 实现异步任务（简历分析、知识库向量化），支持解耦和多语言消费
3. **Gradle**：个人偏好，详见 [Gradle 核心概念总结](https://javaguide.cn/tools/gradle/gradle-core-concepts.html)
...
- **分类管理**：支持互联网大厂、每日更新大厂等多个分类，分类可扩展
- **投递状态追踪**：未投递/筛选中/已测评/已拒绝/无对应岗位，状态颜色标识
- **分页查询**：每页 15 条，支持首页/末页/上下页跳转
- **模糊搜索**：按公司名称实时搜索
- **Excel 导入**：支持 .xlsx/.xls 格式批量导入，自动解析投递链接和发布日期
- **数据管理**：支持添加、编辑、删除、清空操作
...
│   │   │   └── redis/                # Redis 服务、面试会话缓存
│   │   └── modules/                  # 业务模块
│   │       ├── directhire/           # 直达招聘（投递信息管理）
│   │       ├── interview/            # 模拟面试（文字面试、Skill 出题）
│   │       ├── interviewschedule/    # 面试安排（邀请解析、日历管理）
│   │       ├── knowledgebase/        # 知识库（文档上传、向量化、RAG 问答）
│   │       ├── llmprovider/          # 多模型 Provider 管理
│   │       ├── resume/               # 简历管理（上传、解析、分析）
...
复制 `.env.example` 为 `.env`，后端 `bootRun` 会自动读取根目录 `.env`。最少需要填写 `AI_BAILIAN_API_KEY`：
...
使用 `docker-compose.dev.yml` 一键启动 PostgreSQL、Redis、RustFS：
...
> **注意**：首次启动后需访问 [http://localhost:9001](http://localhost:9001) 登录 RustFS 控制台，手动创建名为 `interview-guide` 的 Bucket。
